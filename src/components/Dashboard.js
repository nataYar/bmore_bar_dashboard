...
// }, [])
  ...
  return (
      <div class="w-full bg-gray-100 flex flex-col items-center">
        <div className='w-ful fixed z-10 bg-gray-100 py-3 drop-shadow-md rounded-lg'>
          <div className='w-screen h-12 px-3 flex justify-between lg:justify-end'>
            <button 
             className="w-5/12 lg:w-40 h-12  bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg"
            onClick={ () => addNewCategory() }>Add category</button>
            <button 
            className={ updatedCategories.length >0 || updatedItems.length >0 ? 
            "bg-yellow-400 hover:bg-yellow-500 w-5/12 lg:w-40 rounded-lg text-black font-bold lg:ml-8 " : 
            "rounded-lg w-5/12 lg:ml-8 lg:w-40 cursor-not-allowed  bg-gray-300  text-white font-bold py-2 px-4"}   
        ...
        .map((category) => (
          ...
          className="w-full my-4">
            <a href={`#${category.id}`} className='inline-flex items-center font-medium  hover:underline  hover:decoration-purple-600' >
              ...
              <svg aria-hidden="true" class="w-5 h-5 ml-1" fill="#9333ea" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            </a>
          </div>
          ))
        }
      ...
      .map((category) => (
        ...
        className="w-full my-10 bg-white px-2.5 py-4 rounded-lg">
          {/* <hr class="w-full h-0.5 mx-auto my-8 bg-gray-200 border-0 rounded md:my-10"></hr> */}
